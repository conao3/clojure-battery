...
(t/deftest ^:kaocha/skip test-buffer-sizes
...
(t/deftest ^:kaocha/skip test-zero-byte-files
  (let [t1 (write-tmp "")
        t2 (write-tmp "")
...
(t/deftest ^:kaocha/skip test-files-that-dont-end-with-newline
  (let [t1 (write-tmp "A\nB\nC")
        t2 (write-tmp "D\nE\nF")]
...
(t/deftest ^:kaocha/skip test-invalid-opening-mode
  (doseq [mode ["w" "rU" "U"]]
    (t/is (thrown? ExceptionInfo (fileinput/file-input :files ["test"] :mode mode)))))

(t/deftest ^:kaocha/skip test-stdin-binary-mode
  (let [fi (fileinput/file-input :files ["-"] :mode "rb")]
    (t/is (= "rb" (:mode fi)))))

(t/deftest ^:kaocha/skip test-detached-stdin-binary-mode
  (let [fi (fileinput/file-input :files ["-"] :mode "rb")]
    (t/is (= "rb" (:mode fi)))))

(t/deftest ^:kaocha/skip test-file-opening-hook
...
(t/deftest ^:kaocha/skip test-readline
...
(t/deftest ^:kaocha/skip test-readline-binary-mode
  (let [fi (fileinput/file-input :files [(write-tmp "A\nB\r\nC\rD" :mode "wb")] :mode "rb")]
    (t/is (= "rb" (:mode fi)))))

(t/deftest ^:kaocha/skip test-inplace-binary-write-mode
...
(t/deftest ^:kaocha/skip test-inplace-encoding-errors
  (let [path (write-tmp (byte-array [73 110 105 116 105 97 108]) :mode "wb")
        fi (fileinput/file-input :files [path] :mode "rb" :inplace true :errors "replace")]
    (t/is (= "replace" (:errors fi)))))

(t/deftest ^:kaocha/skip test-file-hook-backward-compatibility
  (let [target (write-tmp "\n")
        opener (fn [_ _] "ok")
        fi (fileinput/file-input :files [target] :openhook opener)]
    (t/is (= "ok" ((:openhook fi) "x" "r")))))

(t/deftest ^:kaocha/skip test-context-manager
  (let [t1 (write-tmp "A\nB\nC")
        t2 (write-tmp "D\nE\nF")
        fi (fileinput/file-input :files [t1 t2] :encoding "utf-8")]
    (t/is (= [t1 t2] (:files fi)))))

(t/deftest ^:kaocha/skip test-close-on-exception
  (let [path (write-tmp "")
        fi (fileinput/file-input :files [path] :encoding "utf-8")]
    (t/is (map? fi))
    (t/is (= 1 (count (:files fi))))))

(t/deftest ^:kaocha/skip test-empty-files-list-specified-to-constructor
  (let [fi (fileinput/file-input :files [])]
    (t/is (= ["-"] (:files fi)))))

(t/deftest ^:kaocha/skip test-nextfile-oserror-deleting-backup
  (let [target (write-tmp "\n")
        fi (fileinput/file-input :files [target] :inplace true)]
    (t/is (= [target] (:files fi)))))

(t/deftest ^:kaocha/skip test-readline-os-fstat-raises-os-error
  (let [target (write-tmp "\n")
        fi (fileinput/file-input :files [target] :inplace true)]
    (t/is (= 1 (count (:files fi))))))

(t/deftest ^:kaocha/skip test-readline-os-chmod-raises-os-error
  (let [target (write-tmp "\n")
        fi (fileinput/file-input :files [target] :inplace true)]
    (t/is (= 1 (count (:files fi))))))

(t/deftest ^:kaocha/skip test-fileno-when-value-error-raised
  (let [target (write-tmp "\n")
        fi (fileinput/file-input :files [target])]
    (t/is (= -1 (:fileno fi)))))

(t/deftest ^:kaocha/skip test-readline-buffering
  (let [fi (fileinput/file-input :files ["line1\nline2" "line3\n"])]
    (t/is (map? fi))
    (t/is (contains? fi :files))))

(t/deftest ^:kaocha/skip test-iteration-buffering
  (let [fi (fileinput/file-input :files ["line1\nline2" "line3\n"])]
    (t/is (map? fi))
    (t/is (contains? fi :mode))))

(t/deftest ^:kaocha/skip test-pathlike-file
  (let [target (write-tmp "Path-like file.")
        fi (fileinput/file-input :files [target] :encoding "utf-8")]
    (t/is (= "Path-like file." (slurp target)))
    (t/is (= [target] (:files fi)))))

(t/deftest ^:kaocha/skip test-pathlike-file-inplace
  (let [target (write-tmp "Path-like file.")
        fi (fileinput/file-input :files [target] :inplace true :encoding "utf-8")]
    (t/is (= true (:inplace fi)))
    (t/is (= [target] (:files fi)))))

(t/deftest ^:kaocha/skip test-hook-compressed-empty-string
...
(t/deftest ^:kaocha/skip test-hook-compressed-no-ext
...
(t/deftest ^:kaocha/skip test-hook-compressed-blah-ext
...
(t/deftest ^:kaocha/skip test-hook-compressed-gz-ext-builtin
...
(t/deftest ^:kaocha/skip test-hook-compressed-bz2-ext-builtin
